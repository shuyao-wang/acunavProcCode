...
%                 or file index of binary file to load (-1 to load more recent file)
...
elseif ~ischar(fname)
    temp = dir('SWIF_AData*.bin');
    if fname == -1
        temp_name = temp(end).name;fname=temp_name;clear temp temp_name
        [basePath,fname] = fileparts(fname);
    else
        temp_name = temp(fname).name;fname=temp_name;clear temp temp_name
        [basePath,fname] = fileparts(fname);
    end
end
